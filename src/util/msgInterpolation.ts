...
const defaultMessageTags: msgTagObj[] = [
  ...
  },
  {
    name: 'msg.guild',
    run: (msg: Message) => {
      return msg.guild ? msg.guild.name : 'This is not a guild'
    },
  },
  {
    name: 'msg.guild.id',
    run: (msg: Message) => {
      return msg.guild ? msg.guild.id : 'This is not a guild'
    },
  },
  {
    name: 'msg.content',
...
export function parseDefaultInterpolator(text: string, msg?: Message) {
  ...
  if (msg) {
    ...
    for (let i = 0; i < defaultActions.length; i++) {
      ...
      if (text.includes(`{${actionsProps.name.trim()}}`)) actionsProps.run(msg)
    }
  }
...
export async function parseCustomInterpolator(
  text: string,
  tagObj: tagObjAny,
  msg?: Message
) {
  let regex = new RegExp(`{${tagObj.name.trim()}}`, 'gi')
  if (msg) text.replace(regex, tagObj.run(msg))
  else text.replace(regex, tagObj.run())
}

export async function parseCustomAction(
  msg: Message,
  tagName: string,
  action: msgActionRun
) {
  if (msg.content.includes(tagName.trim())) action(msg)
}
