...
export class WarnLogger {
  ...
  public async warn(
    ...
    user.createDM(true).then(dm => {
      dm.send(
        simpleEmbed(
          'gold',
          `You were warned in ${user.guild.name} | Case: ${userWarns}`,
          `You were warned for:\n\`\`\`${reason}\`\`\``
        )
      )
    })
  ...
  public async setWarnChannel(msg: Message, id: string) {
    ...
    let possibleGuildWH = (await channel.fetchWebhooks()).find(
      wh => wh.id === dbID
    )
    ...
    if (!wh) {
      channel
        .createWebhook(this.guild.me.displayName, {
      ...
      return msg.channel
        .send(simpleEmbed('green', 'Updated Warn Log channel', ''))
        .catch(e => sendE(msg, e))
    }
  }

  public async resetUserWarnings(
    msg: Message,
    author: GuildMember,
  ...
  public async logMute(
    ...
    const embed = new MessageEmbed()
      .setColor('#99AAB5')
      .setTitle(
        `${`${type.split('')[0].toUpperCase()}${type.substr(
          1,
          type.length
        )}`} | Case: ${cases}`
      )
