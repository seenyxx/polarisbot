...
export async function runCaptcha(member: GuildMember, throwOnFail?: boolean) {
  ...
  if (await db.get(`captcha.${member.guild.id}`)) {
    ...
    let embed = simpleEmbed(
      'pigeon',
      `\`${member.guild.name}\` Captcha`,
      'Please verify your identity, you have 3 minutes to do so (This is case insensitive)'
    )
      .addField(
        'Instructions',
        'Please send the letters/numbers that are in the image provided to verify that you are not a robot. If you do not answer in 3 minutes you will be automatically kicked from the server.'
      )
      .addField('Why?', 'This is to protect this server against raids')
      .setFooter(
        Math.floor(Math.random() * 10) > 6 ? 'How much more easy can it be' : ''
      )
      ...
      if (m.content.toLowerCase() === captcha.text.toLowerCase()) {
        ...
        await m.channel.send(
          simpleEmbed(
            'red',
            'Failed Verification ❌',
            'You will now be kicked from the server'
          )
        )
        if (member.kickable) member.kick('Failed captcha')
        if (throwOnFail) {
