...
export class Leveling {
  ...
  public async add(user?: GuildMember) {
    mongodb.add(
      this.query,
      (await this.xpFormula()) * (user && user?.premiumSince ? 1.2 : 1)
    )
  }
  ...
  public async getColor(): Promise<string> {
    return (await mongodb.get(`colors.${this.userID}`))
      ? await mongodb.get(`colors.${this.userID}`)
      : 'default'
  }
  ...
  public async getGuild(): Promise<Record<string, number>> {
    ...
    return (await mongodb.get(`${this.guildID}`))
      ? await mongodb.get(`${this.guildID}`)
      : emptyObj
  }
  ...
  public async getGuildMulti(): Promise<number> {
    return (await mongodb.get(`multi.${this.guildID}`))
      ? await mongodb.get(`multi.${this.guildID}`)
      : 1
  }
  ...
  public async getLevelingStatus(): Promise<boolean> {
    return (await mongodb.get(`enabled.${this.guildID}`))
      ? await mongodb.get(`enabled.${this.guildID}`)
      : false
  }
}
...
export function checkCoolDown(userID: string, guildID: string) {
  const cd = db.get(`lvlcd${guildID}.${userID}`)
    ? db.get(`lvlcd${guildID}.${userID}`)
    : 0
  const now = Date.now()
...
export async function lvlSetup(
  msg: Message,
  user: GuildMember,
  guildID: string
) {
  ...
  if (!checkCoolDown(user.id, guildID)) {
    ...
    let nextLevel = Math.floor(
      ((await lvl.get()) + (await lvl.xpFormula())) / 1000
    )

    if (nextLevel > currentLevel) {
      msg.channel.send(
        simpleEmbed(
          user.displayHexColor && user.displayHexColor !== '#000000'
            ? user.displayHexColor
            : 'blue',
          '',
          `<@${user.id}> has reached level **${nextLevel + 1}**!`
        )
      )
    }
