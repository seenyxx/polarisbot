...
export async function run(
  ...
  if (!(await user.getLevelingStatus()))
    return message.channel.send('Leveling is not enabled in this server')
  message.channel.send(
    `Are you sure you want to reset <@${member.id}>'s levels?\n\`y\`/\`n\``
  )
  message.channel
    .awaitMessages(m => m.author.id === message.member?.id, {
      max: 1,
      time: 30000,
    })
    .then(messages => {
        ...
        if (msg.content.trim() === 'y') {
          ...
          message.channel.send(
            simpleEmbed(
              'blue',
              'Reset Levels',
              `<@${member.id}>'s levels were reset by <@${message.author.id}>`
            )
          )
        }
