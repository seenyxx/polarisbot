...
export async function run(
  ...
  if (!multi.match(numberRegex))
    return message.channel.send(errorMessage('Invalid number'))
  if (parseFloat(multi) > 8)
    return message.channel.send(
      errorMessage('Multi is too high, the max multi is 8')
    )
  guild.setGuildMulti(parseFloat(multi))

  message.channel.send(
    simpleEmbed('blue', 'Set Multiplier', `Set multiplier to \`${multi}\``)
  )
}
