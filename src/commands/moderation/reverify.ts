...
export function run(client: Client, message: Message, args: Array<string>) {
  ...
  message.channel.send(
    simpleEmbed('pigeon', 'Attempting to ask for reverification..', '')
  )
  ...
  member.createDM().then(async dm => {
    await dm.send(
      simpleEmbed(
        'blue',
        'Type here within 10 minutes to start your verification process',
        ''
      )
    )
    ...
    dm.awaitMessages(m => m, { max: 1, time: 600000 }).then(ms => {
      if (ms.first()) {
        message.channel.send(
          simpleEmbed('green', 'User responded, asking to verify now..', '')
        )

        runCaptcha(member).catch(e =>
          message.channel.send(simpleEmbed('blue', 'Failed verification', e))
        )
      }
