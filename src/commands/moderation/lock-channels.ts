...
export function run(client: Client, message: Message, args: Array<string>) {
  ...
  message.guild.channels.cache.forEach(c => {
    if (!message.guild) return
    if (c.isText() && c.permissionsFor(message.guild.roles.everyone)?.serialize().VIEW_CHANNEL === false) {
      cache.set(c.id, true)
    }
    else if (c.isText() && c.permissionsFor(message.guild.roles.everyone)?.serialize().VIEW_CHANNEL) {
