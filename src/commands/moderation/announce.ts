...
export function run(client: Client, message: Message, args: Array<string>) {
  ...
  const waitForMessage = message.channel.awaitMessages(
  ...
  waitForMessage.then(m => {
    ...
    if (!first)
      return message.channel.send(errorMessage('Announcement not provided'))

    const messageOpts = first.content.split('|')

    if (messageOpts.length !== 2)
      return message.channel.send(errorMessage('Invalid format'))

    const channels = message.guild.channels.cache.filter(c =>
      c.isText()
    ) as Collection<string, TextChannel>
    const pingRole = message.guild.roles.cache.find(r =>
      r.name.toLowerCase().includes('announce')
    )
