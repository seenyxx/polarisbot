...
export async function run(client: Client, message: Message, args: Array<string>) {
  ...
  if (message.member.hasPermission('BAN_MEMBERS')) return message.channel.send(errorMessage('Insufficient Permissions'))
  ...
  if (userArg.match(/[0-9]{17,100}/)) {
    ...
    message.channel.send(simpleEmbed('green', 'Unban', `Unbanned: <@${user.user.id}> | ${user.user.id}\nIssued by: <@${message.member.id}>`))
  }
    ...
    message.channel.send(errorMessage('Invalid format: must be a user id or username#tag'))
  }
}