...
export async function run(client: Client, message: Message, args: Array<string>) {
  ...
  if (!channel?.id) return message.channel.send(errorMessage('No channel provided'))

  let colorMsgs = await message.channel.awaitMessages(filter, {max: 1, time: 30000, errors: ['time'] })
  if (!colorMsgs) return

  let color = colorMsgs.first()?.content
  if (!color?.match(/^#([0-9a-f]{3}|[0-9a-f]{6})$/i)?.length) return message.channel.send(errorMessage('No color provided'))
